...
- 書籍中で統一する表現 :: 表記ゆれチェック
  ...
  #+begin_src yaml :tangle ../config/lhack-tbox.yml
    version: 1

    rules:
      - expected: 安定版
        pattern:  安定板 # 本番誤植で迷惑かけた
      ...
      - expected: パソコン # 本書では「パソコン」で統一
        patterns:
          - PC
          - コンピュータ
          - コンピューター

      - expected: MacOS # 本書では「MacOS」で統一
        patterns:
          - macOS
          - Mac OS
  #+end_src
...
<text>
   1:1    ✓ error  orgmode => Org-mode                                                                                             prh
   5:3    ✓ error  一つ => 1つ
数量を表現し、数を数えられるものは算用数字を使用します。任意の数に置き換えても通用する語句がこれに該当します。  ja-technical-writing/arabic-kanji-numbers
